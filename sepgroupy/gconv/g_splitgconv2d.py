    ...
    def __init__(
        ...
        if self.input_stabilizer_size == 1:
            self.weight = Parameter(
                torch.Tensor(out_channels, in_channels, 1, *kernel_size)
            )
        # Hidden layer
        else:
            self.depthwise_weight = Parameter(
    ...
    def forward(self, input):
        ...
        # Hidden layer
            ...
            if self.separable:
                ...
                tw_shape = (
                    self.out_channels * self.output_stabilizer_size * self.in_channels,
                ...
                # Spatial convolution
                tw_depthwise = trans_filter_d(
                    self.depthwise_weight[:, None, :, :, :], self.input_stabilizer_size
                )
                # Reshape weights
                tw_shape = (
                    self.out_channels * self.output_stabilizer_size,
            ...
            # Non-separable convolution (preconstructed filter)
                ...
                tw = pre_trans_filter(
                    self.depthwise_weight, self.pointwise_weight, self.inds
                )
                tw_shape = (
                    self.out_channels * self.output_stabilizer_size,
                    self.in_channels * self.input_stabilizer_size,
...
class gP4MConvZ2(SplitGConv2D):
    ...
    def __init__(self, *args, **kwargs):
        super(gP4MConvZ2, self).__init__(
...
class gP4MConvP4M(SplitGConv2D):
    def __init__(self, *args, **kwargs):
        super(gP4MConvP4M, self).__init__(
