...
from sepgroupy.gconv import P4ConvZ2, P4ConvP4, GroupCosetMaxPool
...
def conv1x1(in_planes, out_planes, stride=1):
    ...
    """1x1 convolution"""
    return P4ConvP4(in_planes, out_planes, kernel_size=1, stride=stride, bias=False)
...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, base_width=64,
        ...
        if conv_layer is None: conv_layer = P4ConvP4
...
class CustomResNet(ResNet):
    ...
    def __init__(self, block, layers, channels, num_classes=10,
        ...
        self.conv1 = P4ConvZ2(3, self.inplanes, kernel_size=3, stride=1, padding=1, bias=False)
        
        self.bn1 = norm_layer(channels[-1])
        self.relu = nn.ReLU(inplace=True)
        
        self.layer1 = self._make_layer(block, channels[0], layers[0], conv_layer)
        self.layer2 = self._make_layer(block, channels[1], layers[1], conv_layer, stride=2)
        self.layer3 = self._make_layer(block, channels[2], layers[2], conv_layer, stride=2)
...
def p4resnet44(**kwargs):
    return CustomResNet(BasicBlock, [7,7,7], [4,7,14], **kwargs)
